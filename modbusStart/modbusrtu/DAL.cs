using System;
using System.Collections.Generic;
using System.IO;
using System.IO.Ports;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace DAL 
{
    public class modbusrtu
    {
        //建立和断开连接
        //读取线圈 写入线圈
        //定义串口通信对象

        private SerialPort MyCom = new SerialPort();

        //创建串口超时对象
        public int ReadTimeout { get; set; } = 2000;
        public int WriteTimeout { get; set; } = 2000;

        //读取返回报文超时时间

        public int ReceiveTimeOut { get; set; } = 2000;
        /// <summary>
        /// 打开串口
        /// </summary>
        /// <param name="portName"></param>
        /// <param name="baudRate"></param>
        /// <param name="parity"></param>
        /// <param name="dataBits"></param>
        /// <param name="stopBits"></param>
        public void OpenMycom(string portName, int baudRate, Parity parity, int dataBits, StopBits stopBits)
        {
            //若串口打开，先关闭

            if (MyCom.IsOpen)
            {
                MyCom.Close();
            }
            //串口通讯实例化
            MyCom = new SerialPort( portName,  baudRate,  parity,  dataBits,  stopBits);

            //设置超时时间

            MyCom.ReadTimeout = this.ReadTimeout;
            MyCom.WriteTimeout = this.WriteTimeout;
            //打开串口
            MyCom.Open();

        }
        /// <summary>
        /// 关闭串口
        /// </summary>
        public void  CloseMycom()
        {
            if (MyCom.IsOpen)
            {
                MyCom.Close();
            }

        }
        #region 读取线圈输出方法
        /// <summary>
        /// 
        /// </summary>
        /// <param name="iDeviceAdd"></param>
        /// <param name="iAddress"></param>
        /// <param name="iLength"></param>
        /// <returns></returns>
        public byte[] ReadOutputStatus(int iDeviceAdd, int iAddress, int iLength)
        {
            //第一步拼接报文

            byte[] SendCommand = new byte[8];
            SendCommand[0] = (byte)iDeviceAdd;
            SendCommand[1] = 0x01;
            SendCommand[2] = (byte)(iAddress/256);
            SendCommand[3] = (byte)(iAddress %256);
            SendCommand[4] = (byte)(iLength / 256);
            SendCommand[5] = (byte)(iLength % 256);
            byte[] crc = Crc16(SendCommand,6);
            SendCommand[6] = crc[0];
            SendCommand[7] = crc[1];

            //第二步发送报文接收报文
            int ByteLength = 0;
            if (iLength % 8 == 0)
            {
                ByteLength = iLength / 8;


            }
            else
            {
                ByteLength = 1+(iLength / 8);

            }
            byte[] Response = new byte[5 + ByteLength];
            if (SendData(SendCommand, ref Response))  //主站询问报文已经发送，并收到从站回应
            {
                //第三步解析报文
                return GetByteArry(Response, 3, ByteLength);

            }
            else
            { 
                return  null;
            }
            #endregion



        }
        private bool SendData(byte[] SendBety, ref byte[] response)
        {
            try
            {
                MyCom.Write(SendBety, 0, SendBety.Length);
            }
            catch (Exception)
            {
                return false;
            
            }
            //定义一个buffer
            byte[] buffer = new byte[1024];
            //定义一个内存
            MemoryStream ms = new MemoryStream();
            //定义开始读取时间
            DateTime start = DateTime.Now;

            //获取当前缓冲区的值直到读完
            //如果每次都读不到就判断超时
            while (true)
            {
                try
                {
                    if (MyCom.BytesToRead > 1)
                    {
                        int spcount = MyCom.Read(buffer, 0, buffer.Length);
                        ms.Write(buffer, 0, spcount);//读入的数据临时存入内存


                    }
                    else
                    {
                        //超时判断

                        if ((DateTime.Now - start).TotalMilliseconds > this.ReceiveTimeOut)
                        {
                            ms.Dispose();
                            return false;
                        }
                        else if (ms.Length > 0)
                        {
                            break;
                        }
                    
                    }

                }
                catch (Exception)
                {
                    return false;
                    
                }
                           
            }

            response = ms.ToArray();
            ms.Dispose();
            return true;

        }
        /// <summary>
        /// 
        /// </summary>
        /// <param name="dest"></param>
        /// <param name="offset"></param>
        /// <param name="count"></param>
        /// <returns></returns>
        private byte[] GetByteArry(byte[] dest, int offset, int count)
        {
            byte [] res = new byte[count];
            if (res != null && dest.Length >= (offset + count))
            {
                for (int i = 0; i < count; i++)
                {
                    res[i] = dest[offset + i];
                }
                return res;

            }
            else
            {
                return null;
            }

        }

        #region  CRC校验

        private static readonly byte[] aucCRCHi = {
             
         };
        private static readonly byte[] aucCRCLo = {
             
         };
        private byte[] Crc16(byte[] pucFrame, int usLen)
        {
            int i = 0;
            byte[] res = new byte[2] { 0xFF, 0xFF };

            UInt16 iIndex = 0x0000;

            while (usLen-- > 0)
            {
                iIndex = (UInt16)(res[0] ^ pucFrame[i++]);
                res[0] = (byte)(res[1] ^ aucCRCHi[iIndex]);
                res[1] = aucCRCLo[iIndex];
            }
            return res;
        }

        #endregion





    }
}
